...
# fields (SubtreeCrossover.NoOpLog / .KernelLog). Never commit the patched state to
# heal-research/HeuristicLab -- run revert-instrumentation.sh when done.
#
...
if git -C "$HL_DIR" diff --quiet HeuristicLab.Encodings.SymbolicExpressionTreeEncoding/3.4/Crossovers/SubtreeCrossover.cs 2>/dev/null; then
  echo "Applying instrumentation patch to $HL_DIR ..."
  git -C "$HL_DIR" apply "$HERE/subtree-crossover-instrumentation.patch"
else
  echo "SubtreeCrossover.cs already has local changes in $HL_DIR -- assuming instrumentation is already applied, skipping patch."
fi
